...
figlist_qbins = [
    figlist_var_qbins[0],  # .opts(xlabel="", ylabel=""),
    figlist_skewness_qbins[0],  # .opts(xlabel="", ylabel=""),
    figlist_var_qbins[1],  # .opts(xlabel=""),
    figlist_skewness_qbins[1],  # .opts(xlabel=""),
    figlist_var_qbins[2],  # .opts(ylabel=""),
    figlist_skewness_qbins[2],  # .opts(ylabel=""),
]
...
fig_qbins = hv.Layout(figlist_qbins).cols(2).opts(toolbar=None)
fig_qbins_final = fig_qbins.opts(
    hv.opts.HeatMap(frame_height=phelpers.fheight_default, **phelpers.global_kwargs)
)

# hvplot.save(fig_qbins_final, phelpers.fig_dir / "fig_qbins.png")
...
fig_counts = (figlist_var_qbins[3] + figlist_skewness_qbins[3]).opts(toolbar=None)


fig_counts_final = fig_counts.opts(
    hv.opts.HeatMap(
        frame_height=phelpers.fheight_default,
        **phelpers.global_kwargs,
    )
)

# hvplot.save(fig_counts_final, phelpers.fig_dir / "supplemental" / "fig_qbin_counts.png")
